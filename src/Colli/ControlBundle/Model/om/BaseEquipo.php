...
abstract class BaseEquipo extends BaseObject implements Persistent
{
    ...
    /**
     * Clears out the collBodegas collection
     *
    ...
    {
        $this->collBodegas = null; // important to set this to null since that means it is uninitialized
        $this->collBodegasPartial = null;

        return $this;
    }

    /**
     * reset is the collBodegas collection loaded partially
     *
     * @return void
     */
    public function resetPartialBodegas($v = true)
    {
        $this->collBodegasPartial = $v;
    }

    /**
     * Initializes the collBodegas collection.
     *
     * By default this just sets the collBodegas collection to an empty array (like clearcollBodegas());
     ...
     * @return void
     */
    public function initBodegas($overrideExisting = true)
    {
        if (null !== $this->collBodegas && !$overrideExisting) {
            return;
        }
        $this->collBodegas = new PropelObjectCollection();
        $this->collBodegas->setModel('Bodega');
    }

    /**
     * Gets an array of Bodega objects which contain a foreign key that references this object.
     *
     ...
     * @throws PropelException
     */
    public function getBodegas($criteria = null, PropelPDO $con = null)
    {
        $partial = $this->collBodegasPartial && !$this->isNew();
        if (null === $this->collBodegas || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collBodegas) {
                ...
                if (null !== $criteria) {
                    if (false !== $this->collBodegasPartial && count($collBodegas)) {
                      $this->initBodegas(false);

                      foreach ($collBodegas as $obj) {
                        if (false == $this->collBodegas->contains($obj)) {
                          $this->collBodegas->append($obj);
                        }
                      }

                      $this->collBodegasPartial = true;
                    }

                    $collBodegas->getInternalIterator()->rewind();

                    return $collBodegas;
                }

                if ($partial && $this->collBodegas) {
                    foreach ($this->collBodegas as $obj) {
                        if ($obj->isNew()) {
                            $collBodegas[] = $obj;
                        }
                    }
                ...
                $this->collBodegas = $collBodegas;
                $this->collBodegasPartial = false;
            }
        }

        return $this->collBodegas;
    }

    /**
     * Sets a collection of Bodega objects related by a one-to-many relationship
    ...
    {
        $bodegasToDelete = $this->getBodegas(new Criteria(), $con)->diff($bodegas);
        ...
        foreach ($bodegas as $bodega) {
            $this->addBodega($bodega);
        }

        $this->collBodegas = $bodegas;
        $this->collBodegasPartial = false;
    ...
    /**
     * Returns the number of related Bodega objects.
     *
     * @param Criteria $criteria
     * @param boolean $distinct
     * @param PropelPDO $con
     * @return int             Count of related Bodega objects.
     * @throws PropelException
     */
    public function countBodegas(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
    {
        $partial = $this->collBodegasPartial && !$this->isNew();
        if (null === $this->collBodegas || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collBodegas) {
        ...
        return count($this->collBodegas);
    }

    /**
     * Method called to associate a Bodega object to this object
    ...
    public function addBodega(Bodega $l)
    {
        ...
        if (!in_array($l, $this->collBodegas->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            $this->doAddBodega($l);

            if ($this->bodegasScheduledForDeletion and $this->bodegasScheduledForDeletion->contains($l)) {
                $this->bodegasScheduledForDeletion->remove($this->bodegasScheduledForDeletion->search($l));
            }
        }
    ...
    /**
     * @param	Bodega $bodega The bodega object to add.
     */
    ...
    {
        if ($this->getBodegas()->contains($bodega)) {
            $this->collBodegas->remove($this->collBodegas->search($bodega));
            if (null === $this->bodegasScheduledForDeletion) {
    ...
    public function clearAllReferences($deep = false)
    {
        ...
        if ($this->collBodegas instanceof PropelCollection) {
